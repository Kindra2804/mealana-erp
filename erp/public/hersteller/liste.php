...
<script>
var EU_ISO   = <?= $service->getEuLaenderJson() ?>;
var hDaten   = <?= json_encode($hersteller, JSON_UNESCAPED_UNICODE) ?>;
var hAktion  = 'neu';
...
function modalBearbeiten(id) {
    ...
    if (h.logo_pfad) {
        vorschau.src = '/mealana/img/hersteller/' + h.logo_pfad;
...
function modalSchliessen() {
    document.getElementById('h-modal').style.display = 'none';
}
...
async function modalSpeichern() {
    ...
    var url = hAktion === 'neu'
        ? '/mealana/hersteller/speichern.php'
        : '/mealana/hersteller/aktualisieren.php';
    ...
    try {
        ...
        if (data.erfolg) {
            ...
            document.getElementById('h-fehler').textContent =
                Array.isArray(data.fehler) ? data.fehler.join(' · ') : (data.fehler || 'Unbekannter Fehler');
        }
    } catch(e) {
