...
<script>
    ...
    function merkmalSpeichern() {
        ...
        if (!name) {
            fehlerEl.textContent = 'Name darf nicht leer sein';
            return;
        }
        ...
        ajax(action, {
                ...
                name,
                slug,
                mehrfach_auswahl: mehrfach,
                filterbar,
                artikeltyp_ids: typIds
            })
            .then(d => {
                if (d.erfolg) {
                    reload();
                } else {
                    fehlerEl.textContent = d.fehler || 'Fehler';
                    btn.disabled = false;
                }
            });
    }
