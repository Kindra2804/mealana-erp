...
// Altes Bild entfernen, falls schon eins gesetzt war (nur ein Bild pro Kategorie,
// kein Verlauf wie bei Artikel-Bildern nötig)
$altesBild = $repo->findBildPfad($kategorieId);

$uploadDir = __DIR__ . '/../uploads/kategorien/' . $kategorieId . '/';
if (!is_dir($uploadDir)) {
    mkdir($uploadDir, 0755, true);
}

// PNG behält seine Endung (Transparenz), JPEG/WEBP werden als JPEG gespeichert.
// Endung muss VOR dem Verkleinern feststehen, damit Dateiname (für die DB) und
// tatsächlich gespeicherte Datei garantiert übereinstimmen.
$endung    = $mimeTyp === 'image/png' ? 'png' : 'jpg';
...
if ($altesBild && file_exists($uploadDir . $altesBild)) {
    unlink($uploadDir . $altesBild);
}

$repo->updateBild($kategorieId, $dateiname);

echo json_encode([
    'erfolg'    => true,
    'dateiname' => $dateiname,
    'url'       => BASE_PATH . '/uploads/kategorien/' . $kategorieId . '/' . $dateiname,
]);

// ─── Hilfsfunktion (identisch zu bild_upload.php für Artikel) ────────────────
