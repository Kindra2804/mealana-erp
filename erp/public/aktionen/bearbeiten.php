...
foreach ($kundengruppen as $kg) {
    if ($kg['ist_standard']) { $standardKgId = $kg['id']; break; }
}
    ...
    <div style="padding:var(--space-md)">
        ...
        <table class="erp-table" id="kat-tabelle" style="margin-bottom:10px">
            ...
            <?php if (empty($aktion['kategorien'])): ?>
                <tr id="kat-leer-zeile"><td colspan="4" style="color:var(--color-text-muted);font-size:12px">Noch keine Kategorien zugewiesen</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
...
function artikelLaden() {
    ...
    fetch('/mealana/aktionen/aktion_artikel_laden.php?aktion_id=' + AKTION_ID + '&kategorie_id=' + katId + '&kg_id=' + kgId)
        .then(function(r) { return r.json(); })
        .then(function(d) {
            if (!d.erfolg) { inhalt.innerHTML = '<p style="color:var(--color-danger)">' + (d.fehler || 'Fehler') + '</p>'; return; }
...
function renderPreisTabelle(artikel, kgId) {
    ...
    if (hatSubAchsen) {
        Object.values(achsenNamen).forEach(function(n) {
            html += '<th style="text-align:right;white-space:nowrap">' + escH(n) + ' (€ brutto)</th>';
        });
    } else {
        html += '<th style="text-align:right">PREIS (€ brutto)</th>';
    }
    html += '</tr></thead><tbody>';

    artikel.forEach(function(a, ai) {
        html += '<tr>';
        html += '<td style="font-weight:500">' + escH(a.name) + '<div style="font-size:11px;color:var(--color-text-muted)">' + escH(a.artikelnummer) + '</div></td>';
        ...
        if (hatSubAchsen) {
            Object.keys(achsenNamen).forEach(function(achseId) {
                ...
                var val = '';
                if (sa && sa.preis) val = sa.preis.brutto_vk;
                var inputKey = 'preis_' + a.id + '_' + achseId;
                html += '<td style="text-align:right">'
                    + '<input type="text" class="erp-input preis-input" style="width:80px;text-align:right"'
                    + ' data-artikel-id="' + a.id + '" data-sub-achse-id="' + achseId + '" data-mwst="' + (a.mwst_satz || 20) + '"'
                    + ' id="' + inputKey + '" value="' + (val ? parseFloat(val).toFixed(2) : '') + '">'
                    + '</td>';
            });
        } else {
            var val = a.preis ? a.preis.brutto_vk : '';
            html += '<td style="text-align:right">'
                + '<input type="text" class="erp-input preis-input" style="width:80px;text-align:right"'
                + ' data-artikel-id="' + a.id + '" data-sub-achse-id="" data-mwst="' + (a.mwst_satz || 20) + '"'
