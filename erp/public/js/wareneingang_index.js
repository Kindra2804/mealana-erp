        ...
        .then(function (data) {
            ...
            if (!data.gefunden) {
                box.innerHTML = '<div style="color:var(--color-danger);font-size:13px">EAN nicht gefunden: ' + escHtml(ean) + '</div>' +
                    '<a href="' + window.BASE_PATH + '/wareneingang/artikel_vorbereiten.php?ean=' + encodeURIComponent(ean) + '" class="btn btn-secondary btn-sm" style="margin-top:8px">+ Neuen Artikel anlegen</a>';
            ...
            if (a.hauptbild) {
                html += '<img src="' + window.BASE_PATH + '/uploads/artikel/' + a.id + '/' + escHtml(a.hauptbild) + '" style="width:72px;height:72px;object-fit:cover;border-radius:4px;flex-shrink:0" onerror="this.style.display=\'none\'">';
            }
            ...
            if (!bs.length) {
                ...
                html += '<a href="' + window.BASE_PATH + '/lager/wareneingang.php?ean=' + encodeURIComponent(ean) + '" style="display:block;margin-top:8px;font-size:12px;color:var(--color-text-muted)">Freier Wareneingang ohne Bestellung</a>';
            }
            html += '</div></div>';
            box.innerHTML = html;
            box.style.display = 'block';
...
function zurSammelliste(artikelId, name, eanEncoded) {
    ...
    fd.append('name', name);
    fd.append('ean', ean);
    fetch(window.BASE_PATH + '/wareneingang/durchlauf_add.php', { method: 'POST', body: fd })
        .then(function (r) { return r.json(); })
        .then(function (d) { if (d.erfolg) window.location.reload(); });
}

function durchlaufLeeren() {
    fetch(window.BASE_PATH + '/wareneingang/durchlauf_clear.php', { method: 'POST' })
        .then(function (r) { return r.json(); })
