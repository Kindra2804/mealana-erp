...
- **MeaLana-Entscheidung (Stand 2026-07-06, korrigiert):** Die eigene Nullbeleg-Logik bleibt bewusst als Redundanz bestehen ("besser doppelt als vergessen"). Die eigene **Nachsignierungs**-Logik wurde dagegen nach einem echten Hardware-Test komplett entfernt (Migration 112) — sie war nicht nur redundant, sondern barg ein echtes RKSV-Reihenfolge-Risiko. Siehe [[project_rksv_bfr]] für die neue State-Check-Gate-Architektur.
- **Wichtigste Klarstellung aus der echten API-Doku (`BFR_API.html`, 2026-07-06):** `RC` bei `/register` ist **immer** `"OK"` — nur `<Link>` unterscheidet echte Signatur von "Sicherheitseinrichtung ausgefallen". Das war der ursprüngliche Denkfehler, der zur alten (jetzt entfernten) Nachsignierungs-Architektur geführt hat.
