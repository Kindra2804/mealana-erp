...
**Auslöser:** Jacky meldete SCHEEPJES UNITY (DB-1710) zeigt trotz augenscheinlich korrekter Daten (grundpreis_anzeigen=1, inhalt_menge=100g, längst synct) keine Grundpreisauszeichnung im Shop -- Vermutung: Warteschlangen-Reihenfolge beim Komplettabgleich. Live-Recherche direkt gegen die WooCommerce-API (nicht nur eigene DB) widerlegte das.

**Root Cause (echte API-Eigenheit von WooCommerce/Germanized, keine Reihenfolge-Frage):** Direkter GET auf die Live-Variation zeigte `unit_price` teilweise befüllt (`product:"100", price:"10.95"`), aber `unit: null` UND `unit_price.base: ""` leer. Gezielte isolierte PUT-Tests bestätigten: WooCommerce/Germanized nimmt einen PUT mit `unit`/`unit_price.base` direkt an eine Variation mit 200 OK an, verwirft beide Felder aber komplett -- kein Fehler, keine Warnung, einfach still ignoriert. Identische PUTs ans ELTERNPRODUKT dagegen funktionieren sofort. Sprich: **`unit` (welche Einheit) UND `unit_price.base` (die Bezugsmenge, z.B. 100) müssen am Vater sitzen -- nur `unit_price.product`/`price`/`price_regular` (die variationsspezifischen Zahlen) gehören an die Variation.** Eine nirgends dokumentierte Asymmetrie, die die bisherige Verifizierung (2026-07-23, an einem Standalone-Testartikel D-1059 ohne Achsen) nie aufgedeckt hat, weil dort Vater und "Variation" derselbe Datensatz sind.

**Zwei Fix-Runden nötig, weil die erste unvollständig war:** Runde 1 hat nur `unit` an den Vater verlagert (neue Methode `baueGrundpreisEinheitFeld()`) -- live getestet, `unit` saß korrekt am Vater, aber SCHEEPJES zeigte im Shop-Frontend TROTZDEM nichts an (von Jacky richtig eingefordert: "wenn Carosello, das nie synct wurde, richtig aussieht, aber das angeblich gefixte Scheepjes immer noch nichts zeigt, stimmt was nicht"). Vergleich mit Carosello (AD-CS, nie über unseren Sync gelaufen, manuell in WordPress eingetragen, zeigt korrekt an) deckte auf: dort ist am Vater AUCH `unit_price.base="100"` gesetzt -- das fehlte in Runde 1. Runde 2: `baueGrundpreisEinheitFeld()` umbenannt zu `baueGrundpreisVaterFelder()`, liefert jetzt `unit` UND `unit_price.base` gemeinsam.

**Bestehender Code-Bug, der das verschärfte:** `ShopSyncService::baueProduktPayload()` hat bei Variable Products (Vater mit Achsen) `baueGrundpreisFelder()` komplett übersprungen (gleiches `empty($dimensionen)`-Gate wie beim schon bekannten Bestands-Bug vom 2026-08-01) -- beide Felder bekamen am Vater also nie überhaupt die Chance, gesetzt zu werden.

**Fix (finaler Stand):** `baueGrundpreisFelder()` (Standalone-Artikel + jede Kind-Variation, unverändert) und die neue `baueGrundpreisVaterFelder()` (nur `unit`+`unit_price.base`, für den Vater bei Variable Products) teilen sich jetzt eine gemeinsame Eligibilitäts-Prüfung `ermittleGrundpreisBasis()`. `baueProduktPayload()` ruft `baueGrundpreisVaterFelder()` im Variable-Product-Zweig auf (dort, wo bisher nur `manage_stock=false` gesetzt wurde).

**Live end-to-end verifiziert (nicht nur Code gelesen):** Kompletter Payload für DB-1710 über den echten Code-Pfad gebaut, per PUT an WooCommerce geschickt, direkt danach per GET bestätigt -- Vater hat jetzt `unit` UND `unit_price.base` korrekt, UND die Variation selbst zeigt jetzt ein vollständig gerendertes `unit_price.price_html` ("10,95 € / 100 g", identisches Format zur funktionierenden Carosello-Referenz). **480 Väter** (alle mit Achsen + aktivem Grundpreis) als fällig markiert (`aktualisiert_am` gebumpt) -- geprüft, dass währenddessen keiner davon bereits mit der unvollständigen Runde-1-Version resynct wurde (0 Treffer im Zeitfenster), alle 480 bekommen also den vollständigen Fix beim nächsten Durchlauf.

**Praktische Konsequenz fürs Verständnis vom Sync-Verhalten:** Jackys ursprüngliche Sorge war berechtigt formuliert ("werden Artikel bei jeder Änderung hinten angereiht?") -- die Antwort ist NEIN im zeitlichen Sinn (die `while(true)`-Schleife in `komplettabgleich.php` arbeitet einen einzelnen Lauf komplett bis "nichts mehr fällig" durch), aber die Sortierung innerhalb eines Laufs ist rein nach `vaterartikel_id`/`id` aufsteigend (kein "neueste zuerst") -- ein frisch fälliger Artikel mit hoher ID kann daher hinter einem großen Rückstau niedrigerer IDs warten, bis der abgearbeitet ist.

**Lehre:** Ein Live-API-Vergleich mit einem bekannt-funktionierenden Referenzartikel (hier: Carosello, zufällig nie über unseren Sync gelaufen und daher ein "sauberes" Beispiel für den Zielzustand) deckte den unvollständigen ersten Fix auf, den reines Nachlesen des eigenen Codes nicht gefunden hätte -- bei WC/Germanized-API-Eigenheiten lohnt sich der direkte Feld-für-Feld-Vergleich gegen ein echtes Positiv-Beispiel.
